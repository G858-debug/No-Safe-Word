/**
 * No Safe Word — Flux 2 Dev ControlNet Pose Test
 * ================================================
 * Tests Alibaba's FLUX.2-dev-Fun-ControlNet-Union for pose-controlled
 * explicit image generation with Flux 2 Dev on RunPod.
 *
 * Generates reference portraits for two characters (Thabo + Naledi),
 * then runs pose-controlled couple scenes at varying ControlNet strengths
 * across multiple explicit positions.
 *
 * Character consistency via detailed prompt descriptions. PuLID Flux is
 * incompatible with ComfyUI 5.8.x's wrapper chain (multiple unfixable
 * integration issues with Flux2Fun ControlNet).
 *
 * Usage:
...
// ─────────────────────────────────────────────
// CHARACTERS — Thabo (male) and Naledi (female) from Middelburg Nights
// ─────────────────────────────────────────────
...
const TESTS: TestCase[] = [
  { id: "01_baseline", name: "Baseline (no ControlNet)", poseId: "missionary", poseStrength: null, prompt: PROMPTS.missionary },
  { id: "02_pose_040", name: "Missionary — pose 0.40", poseId: "missionary", poseStrength: 0.4, prompt: PROMPTS.missionary },
  { id: "03_pose_060", name: "Missionary — pose 0.60", poseId: "missionary", poseStrength: 0.6, prompt: PROMPTS.missionary },
  { id: "04_pose_070", name: "Missionary — pose 0.70", poseId: "missionary", poseStrength: 0.7, prompt: PROMPTS.missionary },
  { id: "05_pose_080", name: "Missionary — pose 0.80", poseId: "missionary", poseStrength: 0.8, prompt: PROMPTS.missionary },
  { id: "06_cowgirl", name: "Cowgirl — pose 0.70", poseId: "cowgirl", poseStrength: 0.7, prompt: PROMPTS.cowgirl },
  { id: "07_from_behind", name: "From Behind — pose 0.70", poseId: "from-behind", poseStrength: 0.7, prompt: PROMPTS["from-behind"] },
  { id: "08_standing_lift", name: "Standing Lift — pose 0.70", poseId: "standing-lift", poseStrength: 0.7, prompt: PROMPTS["standing-lift"] },
];
...
function buildPoseWorkflow(prompt: string, seed: number, poseImg: string, strength: number): Record<string, any> {
  return {
    "1": { class_type: "UNETLoader", inputs: { unet_name: UNET, weight_dtype: "fp8_e4m3fn" } },
    "2": { class_type: "CLIPLoader", inputs: { clip_name: TEXT_ENCODER, type: "flux2" } },
    "3": { class_type: "VAELoader", inputs: { vae_name: VAE } },
    ...
    "10": { class_type: "BetaSamplingScheduler", inputs: { model: ["1", 0], steps: STEPS, alpha: 0.6, beta: 0.95 } },
    "11": { class_type: "BasicGuider", inputs: { model: ["1", 0], conditioning: ["6", 0] } },
...
async function generatePortrait(name: string, prompt: string, seed: number): Promise<Buffer> {
  ...
  return base64ToBuffer(imageBase64);
}
...
function generateReport(results: TestResult[], thaboFile: string, nalediFile: string): string {
  ...
  function makeCard(r: TestResult): string {
    ...
    const label = r.poseStrength !== null ? `Pose ${r.poseStrength.toFixed(2)}` : "No CN";
    ...
    return `<div class="card"><div class="card-images"><div class="card-img">${imgTag}</div>${poseThumb}</div>
      <div class="card-info"><div class="badges"><span class="str-badge">${label}</span>
      <span class="badge ${r.success ? "pass" : "fail"}">${badge}</span></div>
      <h3>${r.name}</h3><p class="prompt">${r.prompt.slice(0, 120)}...</p></div></div>`;
  }

  const sweep = results.filter((_, i) => i < 5);
  const variety = results.filter((_, i) => i >= 5);

  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Flux 2 Dev \u2014 ControlNet Pose Test</title>
...
<style>:root{--bg:#0a0607;--surface:#1a1012;--crimson-light:#c4384f;--amber-light:#f5c542;--text:#e8ddd0;--text-muted:#8a7d72;--radius:8px}*{margin:0;padding:0;box-sizing:border-box}body{background:var(--bg);color:var(--text);font-family:'DM Sans',sans-serif;line-height:1.6}.header{text-align:center;padding:60px 20px 40px;background:linear-gradient(180deg,#1a0a0e,var(--bg));border-bottom:1px solid rgba(139,26,43,.3)}.header h1{font-family:'Playfair Display',serif;font-size:2.4rem;background:linear-gradient(135deg,var(--amber-light),var(--crimson-light));-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:8px}.header .sub{color:var(--text-muted);margin-bottom:24px}.stats{display:flex;justify-content:center;gap:40px;margin-top:20px}.stat{text-align:center}.stat .n{font-family:'Playfair Display',serif;font-size:2rem;font-weight:700;color:var(--amber-light)}.stat .l{font-size:.8rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:1px}.section{max-width:1400px;margin:0 auto;padding:40px 20px}.section h2{font-family:'Playfair Display',serif;font-size:1.8rem;margin-bottom:8px;color:var(--amber-light)}.section .desc{color:var(--text-muted);margin-bottom:24px}.ref-row{display:flex;gap:20px;justify-content:center;flex-wrap:wrap;margin-bottom:32px}.ref-card{background:var(--surface);border-radius:var(--radius);overflow:hidden;border:1px solid rgba(139,26,43,.2);width:220px}.ref-card img{width:100%;aspect-ratio:832/1216;object-fit:cover}.ref-card .lab{padding:10px;text-align:center;font-family:'Playfair Display',serif;color:var(--amber-light)}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(380px,1fr));gap:20px}.card{background:var(--surface);border-radius:var(--radius);overflow:hidden;border:1px solid rgba(139,26,43,.15);transition:border-color .2s,transform .2s}.card:hover{border-color:rgba(139,26,43,.4);transform:translateY(-2px)}.card-images{display:flex}.card-img{flex:1;aspect-ratio:832/1216;overflow:hidden;background:#110a0c;display:flex;align-items:center;justify-content:center}.card-img img{width:100%;height:100%;object-fit:cover;cursor:pointer;transition:transform .3s}.card-img img:hover{transform:scale(1.03)}.pose-thumb{width:120px;background:#0d0809;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:8px;border-left:1px solid rgba(139,26,43,.1)}.pose-thumb img{width:100%;border-radius:4px;opacity:.85}.pose-thumb span{font-size:.65rem;color:var(--text-muted);margin-top:4px;text-align:center}.pose-thumb .no-pose{font-size:.7rem;color:var(--text-muted)}.card-info{padding:16px}.card-info h3{font-family:'Playfair Display',serif;font-size:1rem;margin-bottom:8px}.prompt{font-size:.8rem;color:var(--text-muted);margin-bottom:12px;line-height:1.5}.badges{display:flex;gap:8px;margin-bottom:8px}.badge{padding:4px 12px;border-radius:20px;font-size:.75rem;font-weight:600}.badge.pass{background:rgba(45,106,79,.2);color:#6fcf97;border:1px solid rgba(45,106,79,.4)}.badge.fail{background:rgba(139,26,43,.2);color:var(--crimson-light);border:1px solid rgba(139,26,43,.4)}.str-badge{padding:4px 12px;border-radius:20px;font-size:.75rem;font-weight:600;background:rgba(212,146,10,.15);color:var(--amber-light);border:1px solid rgba(212,146,10,.3)}.filtered{width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;color:var(--crimson-light);font-weight:600;padding:20px;text-align:center}.filtered span{font-size:.75rem;color:var(--text-muted);margin-top:8px}.modal-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.92);z-index:1000;align-items:center;justify-content:center;padding:20px;cursor:pointer}.modal-overlay.active{display:flex}.modal-content{max-width:90vw;max-height:90vh;display:flex;gap:30px;align-items:flex-start;cursor:default}.modal-content img{max-height:85vh;max-width:55vw;object-fit:contain;border-radius:var(--radius)}.modal-details{max-width:400px}.modal-details h3{font-family:'Playfair Display',serif;font-size:1.3rem;margin-bottom:12px;color:var(--amber-light)}.modal-details .full-prompt{font-size:.85rem;color:var(--text-muted);line-height:1.7;white-space:pre-wrap}@media(max-width:768px){.header h1{font-size:1.8rem}.grid{grid-template-columns:1fr}.modal-content{flex-direction:column}.modal-content img{max-width:90vw}.pose-thumb{width:80px}.ref-row{flex-direction:column;align-items:center}}</style></head><body>
<div class="header"><h1>Flux 2 Dev \u2014 ControlNet Pose Test</h1>
<p class="sub">Pose control via Flux2Fun ControlNet on RunPod \u2014 ${dateStr}</p>
...
<div class="section"><h2>Character References</h2>
<p class="desc">Portraits generated with Flux 2 Dev. Character consistency maintained via detailed prompt descriptions across all scene tests.</p>
<div class="ref-row">
<div class="ref-card"><img src="${thaboFile}" /><div class="lab">Thabo (male)</div></div>
<div class="ref-card"><img src="${nalediFile}" /><div class="lab">Naledi (female)</div></div>
</div></div>
<div class="section"><h2>Strength Sweep (Missionary)</h2>
<p class="desc">Same prompt and seed. Baseline (no CN) vs pose strengths 0.40\u20130.80.</p>
<div class="grid">${sweep.map(makeCard).join("\n")}</div></div>
<div class="section"><h2>Pose Variety (strength 0.70)</h2>
<p class="desc">Different explicit positions with ControlNet at 0.70 — cowgirl, from-behind, standing-lift.</p>
...
async function main() {
  ...
  console.log("  Flux 2 Dev \u2014 ControlNet Pose Test");
  console.log("=".repeat(60));
  ...
  for (let i = 0; i < TESTS.length; i++) {
    const test = TESTS[i];
    const seed = i < 5 ? baseSeed : baseSeed + i;
    console.log(`\n\u2501\u2501\u2501 [${i + 1}/${TESTS.length}] ${test.name} \u2501\u2501\u2501`);

    const filename = `${test.id}.png`;
    try {
      let workflow: Record<string, any>;
      let images: Array<{ name: string; image: string }> | undefined;
      let poseFilename: string | null = null;

      if (test.poseStrength === null) {
        ...
        workflow = buildPoseWorkflow(test.prompt, seed, pd.filename, test.poseStrength);
        images = [{ name: pd.filename, image: pd.buffer.toString("base64") }];
        console.log(`  Mode: pose (${test.poseId} @ ${test.poseStrength})`);
      }
  ...
  for (const r of results) {
    ...
    const cn = r.poseStrength !== null ? `pose ${r.poseStrength}` : "baseline";
    console.log(`  ${icon} ${r.id}: [${cn}] [${time}]`);
  }

...
import { generateSdxlBodyShots } from "@no-safe-word/image-gen/server/character-lora/dataset-generator";
...
export async function POST(
  ...
  try {
    ...
    (async () => {
      try {
        const sdxlResult = await generateSdxlBodyShots(characterInput, lora.id, 16, { supabase });

    ...
    for (let i = 0; i < prompts.length; i++) {
      ...
      try {
        ...
        if (!imgPrompt.character_id) {
          ...
          console.log(`[StoryImage][${imgPrompt.id}] Dual-character scene: using dual-character workflow (IPAdapter, no LoRAs)`);
          ...
          console.log(`[StoryImage][${imgPrompt.id}] Prompt describes ${promptCharCount} characters but no secondary_character_id and no LoRA — using single-character workflow`);
        } else {
